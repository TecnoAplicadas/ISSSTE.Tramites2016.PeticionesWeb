...
namespace ISSSTE.Tramites2015.Common.Security.Core
{
    ...
    public class IsssteUser
    {
        ...
        /// </summary>
        /// <returns>Lista de ids de delegaciones</returns>
        public List<IsssteDelegacionSADProperty> GetDelegationsSAD()
        {
            var DelegacionesSAD = this.GetPropertyValues(IsssteUserPropertyTypes.DelegationSAD);

            var resultado = DelegacionesSAD.Select(s =>
             new IsssteDelegacionSADProperty
             {
                 DelegationName = s.Name,
                 IdDelegationSAD = Convert.ToInt32(s.Value),
                 UPS = s.Atributes==null?new List <IsssteUserProperty>(): JsonConvert.DeserializeObject<List<IsssteUserProperty>>(s.Atributes),
