(function () {
    ...
    appMod.controller('RequestController', ['$q', '$http', '$window', '$filter', '$anchorScroll', '$location', '$timeout', function ($q, $http, $window, $filter, $anchorScroll, $location, $timeout) {
        ...
        var promiseAreaColl = $http.get('../SolicitudPeticion/GetAreas'); //Catálogo de Areas        
        ...
        vm.getUnitColl = function (pIdDelegation) {
            debugger;
            vm.requestUnitColl = [];

            if (pIdDelegation !== undefined) {

                var data = { pIdUnidadAdministrativa: pIdDelegation };
        ...
        //Metodo para obtener catalogo de causa del asunto por tipo de opinion y area
        vm.getCauseCollPorTipoOpinionYArea = function (pIdTipoOpinion, pIdArea) {
            ...
            if (pIdTipoOpinion !== undefined && pIdArea !== undefined) {
                vm.causePromise = $http.post('../SolicitudPeticion/GetCausaAsuntoPorTipoOpinionYArea', data).then(function (response) {
                    vm.subjectCauseColl = response.data;
                }, function (response) {
                    vm.errorList = [];
                    vm.errorList.push("No se pudo recuperar catalogo de causas del asunto.");
        ...
        //Método para obtener el catálogo de servicios donde ocurrieron los hechos
        vm.getCauseServiciosColl = function (pIdTipoOpinion, pIdArea) {
            ...
            vm.promisePlaceColl = $http.post('../SolicitudPeticion/GetServicioHechosPorArea', dataServ).then(function (response) {
                vm.requestPlaceColl = response.data;
            });
            ...
            if (form.$valid) {
                ...
                if (captchaValidation.length === 0) {
                    ...
                    dataPost = {
                        ...
                        Area: form.areaPeticionSlct.$modelValue.IdArea,
